/**
 * @file
 * @brief
 * Lametta checksum
 * 
 * @details
 * This software is provided under public domain without any warranty.
 */
#include <tcl.h>
#include <stdint.h>


/**
 * @brief
 * Seed value
 */
#define LAMETTA_SEED 0xA5u


/**
 * @brief
 * S-box
 */
static uint8_t T[256] =
{
    
};


static int Lametta_Compute(ClientData data, Tcl_Interp* interp, int objc, Tcl_Obj* const objv[])
{
    unsigned char* bytes;
    unsigned int i;
    unsigned int y;
    int result;
    int length;
    int seed;
    
    (void)data;
    
    result = TCL_ERROR;
    
    /* ::lametta::compute x {seed 0xa5} */
    if ((objc == 2) || (objc == 3))
    {
        /* Get seed value */
        y = LAMETTA_SEED;
        if (objc >= 3)
        {
            if (TCL_OK == Tcl_GetIntFromObj(interp, objv[2], &seed))
            {
                if (seed > 0xff)
                {
                    Tcl_SetObjResult(interp, Tcl_NewStringObj("Bad seed value", -1));
                    return TCL_ERROR;
                }
                y = 0xFFu & ((unsigned int)seed);
            }
        }
        bytes = Tcl_GetByteArrayFromObj(objv[1], &length);
        if (NULL != bytes)
        {
            /* Unrolling this loop hasn't improved runtime so we keep it simple */
            for (i = 0u; i < (unsigned int)length; ++i)
            {
                y = T[0xFFu & (y + bytes[i])];
            }
        }
        Tcl_SetObjResult(interp, Tcl_NewIntObj(y));
        result = TCL_OK;
    }
        else
        {
            Tcl_WrongNumArgs(interp, 1, objv, NULL);
        }
    
    return result;
}


DLLEXPORT int Lametta_Init(Tcl_Interp* interp)
{
    int result;
    
    result = TCL_ERROR;
    
    if (NULL != Tcl_InitStubs(interp, "8.6", 0))
    {
        if (TCL_ERROR != Tcl_PkgProvide(interp, "lametta", "1.0.0"))
        {
            Tcl_CreateObjCommand(interp, "::lametta::compute", Lametta_Compute, NULL, NULL);
            result = TCL_OK;
        }
    }
    
    return result;
}
